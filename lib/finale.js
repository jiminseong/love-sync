...
// 공유용 URL은 punycode(xn--h43baa.kro.kr) 대신 한글 도메인으로 고정.
// 받는 사람이 punycode를 피싱처럼 의심하는 인식 이슈가 있어서다.
// 도메인 자체는 같지만 한국 메신저(카톡/iMessage)가 한글 도메인을 자동
// 링크화·OG 미리보기까지 잘 받아주는 걸 확인하고 둔다. 도메인이 바뀌면
// 여기도 같이 손대야 함.
const SHARE_URL = 'https://뽀뽀뽀.kro.kr/';
 ...
 * className: 추가 클래스
 */
function isDynamicFinaleSegments(segments) {
  if (!Array.isArray(segments) || segments.length !== 6) return false;
  return segments.every((segment, index) => {
    const expectedTag = index === 0 ? 'h2' : 'p';
    return Boolean(
      segment &&
      segment.tag === expectedTag &&
      typeof segment.className === 'string' &&
      Array.isArray(segment.lines) &&
      segment.lines.length > 0 &&
      segment.lines.length <= 3 &&
      segment.lines.every((line) => typeof line === 'string' && line.trim())
    );
  });
}

function normalizeDynamicSegments(segments) {
  if (!isDynamicFinaleSegments(segments)) return null;
  return segments.map((segment) => ({
    ...
    lines: segment.lines
      .flatMap((line, index) => {
        ...
        if (segment.tag === 'h2' && index === 0 && trimmed.includes('액정에 ')) {
          return trimmed.replace('액정에 ', '액정에\n').split('\n');
        }
        return [trimmed];
      })
      .slice(0, 3),
...
export function buildFinaleSegments(deviceName, dynamicSegments = null) {
  const normalizedDynamicSegments = normalizeDynamicSegments(dynamicSegments);
  if (normalizedDynamicSegments) return normalizedDynamicSegments;
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      if (navigator.share) {
        ...
        track('share_completed', { method: 'web_share' });
        return;
      }
    } catch (_err) {
...
export async function runFinale({
  ...
  const segments = buildFinaleSegments(deviceName, dynamicSegments);
