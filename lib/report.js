...
export function generateReport(result, { enhanced } = {}) {
  ...
  const prescription =
    typeof enhanced?.prescription === 'string' && enhanced.prescription.trim()
      ? enhanced.prescription.trim()
      : VIBES[result.personality] ?? VIBES.dependent;
...
export function renderReportBodyHTML(report) {
  const metricsRows = report.metrics
    .map(
      (m) =>
        `<div class="metric-row"><span class="metric-row__label">${escapeHTML(m.label)}</span><span class="metric-row__dots" aria-hidden="true"></span><span class="metric-row__value">${escapeHTML(m.value)}</span></div>`
    )
    .join('');

  const performanceItems = report.performance
    .map((line) => `<li class="perf-item">${escapeHTML(line)}</li>`)
    .join('');

  return `
    <section class="report-section report-section--metrics">
      <p class="caption">${escapeHTML(report.headline)}</p>
    ...
    <section class="report-section report-section--prescription">
      ...
      <blockquote class="prescription">${escapeHTML(report.prescription)}</blockquote>
    </section>
  `;
}
